	...
	<div>
		...
		<div class="goodsCon">
			<div class="goodTab">
				<ul class="clearfix">
					<li class="active">
						<router-link :to="{path:'/user/purordlist?newpage=newpage'}">大宗商品</router-link>
					</li>
					<li>
						<router-link :to="{path:'/user/purcommordlist?newpage=newpage'}" >普通商品</router-link>
					</li>
				</ul>
			</div>
			<div class="ordList" v-for="(item,index) in orderVOS" :key="item.id" v-show="tabShow">				
	...
	export default {
		...
		data() {
			return {
				...
				numPerPage: 10,
				status: 1,
				function: 'commoditySupplyOrderList',
				totalCount: '',
				blshow: false,
				tabShow: true,
				tabcommShow: false,
