<template>
  <div class="flagshop" >
    <div :class="{headerBg:!titleFixed}">
      ...
      >
        ...
        <div
          class="searchBox"
          @click="search"
          :class="titleFixed ? 'searchHeaderBgColorFixed' : 'searchHeaderBgColor'"
        >
          <icon
            type="search"
            class="searchIco"
            :class="titleFixed ? 'headerColor' :'searchIcoColor'"
          ></icon>
          <input
            type="text"
            placeholder="请输入关键字"
    ...
    <div class="pride_tab_fixed sticky" ref="pride_tab_fixed">
      <div class="nav" :class="titleFixed == true && noios ? 'isFixed' :''">
        <sticky scroll-box="vux_view_box_body" :check-sticky-support="false" :offset="46">
          <tab :line-width="3" active-color="#ff4f54">
            <tab-item
              :selected="curNav === item"
    ...
    <div class="list" ref="list">
      <div
        class="introBox listItem"
        v-show="curHome && newMallList.length == 0 && projectList.length == 0 && meetingList.length == 0 && homeIntro"
      >
        <div class="titleBox">
    ...
    <div class="footer">
      <div
        class="componyIntroBtn"
        :class="showTypeFooter ? 'btnWidthPf' : 'btnWidth'"
        @click="goComponyIntro"
      >公司介绍</div>
      <!--<div class="typeBtn" v-show="showTypeFooter">
				分类
      </div>-->
      <div class="leaveSpeakeBtn" @click="goLeaveSpeak">留言</div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed:{
    getHeaderC(){
      return this.titleFixed ? 'searchHeader1' : 'searchHeader'
    }
  },
  methods: {
    ...
    touchmove(e){
        this.offsetTopNav = this.$refs.pride_tab_fixed.getBoundingClientRect().top;
        this.headerH = this.$refs.searchHeader.offsetHeight;
        this.titleFixed = (this.offsetTopNav <= this.headerH);
    },
    handleScroll() {
      this.offsetTopNav = this.$refs.pride_tab_fixed.getBoundingClientRect().top;
    ...
    follow() {
      ...
      axios
        .get(this.$root.urlPath.MC + "/wap/store.do?storeAttention", {
          params: {
            storeId: this.shopId
          }
        })
    ...
    followed() {
      ...
      axios
        .get(this.$root.urlPath.MC + "/wap/store.do?storeCancelAttention", {
          params: {
            storeId: this.shopId
          }
        })
    ...
    getProject(params) {
      this.loading = true;
      axios
        .get(this.$root.urlPath.MC + "/wap/store.do?yiLaList", {
          params
        })
        .then(res => {
          this.loading = false;
    ...
    getMeeting(params) {
      this.loading = true;
      axios
        .get(this.$root.urlPath.MC + "/wap/store.do", {
          params
        })
        .then(res => {
          this.loading = false;
    ...
    getComponyInfo() {
      axios
        .get(this.$root.urlPath.MC + "/wap/store.do?storeInfo", {
          params: {
            id: this.shopId
          }
        })
    ...
    getNewMallList(params) {
      this.loading = true;
      axios
        .get(this.$root.urlPath.MC + "/wap/store.do?mallLatest", {
          params
        })
        .then(res => {
          this.loading = false;
          if (res.data.success) {
            ...
            var reg = /^(.*\.)[^.]+$/;
            for (
              var i = (this.pageNum - 1) * 10;
              i < this.newMallList.length;
              i++
            ) {
              this.$set(
                this.newMallList[i],
                "imgS",
                this.newMallList[i].img.replace(reg, "$1") + "s.jpg"
              );
              this.$set(
                this.newMallList[i],
                "imgM",
                this.newMallList[i].img.replace(reg, "$1") + "m.jpg"
              );
            }
          } else {
            //							alert(res.data.errMsg)
    ...
    getMallList(params) {
      ...
      axios
        .get(this.$root.urlPath.MC + "/wap/store.do?mallList", {
          params
        })
        .then(res => {
          this.loading = false;
    ...
    getHomeIntro() {
      axios
        .get(this.$root.urlPath.MC + "/wap/store.do?briefIntroduction", {
          params: {
            storeId: this.shopId
          }
        })
    ...
    goLeaveSpeak() {
      if (!cookie.get("MC_UID")) {
        ...
        this.$axios
          .get(this.$root.urlPath.MCT + "/wap/company/comInfo")
          .then(res => {
            var data = res.data;
            ...
            if (data.status == 2) {
              ...
              this.$vux.confirm.show({
                ...
                onConfirm() {
                  window.location.href =
                    "http://trade.miningcircle.com/cert?newpage=newpage";
                }
              });
            } else if (data.status == 1) {
              this.$vux.toast.text(
                "您的企业正在申请中<br/>客服：400-819-6985",
                "center"
              );
            } else if (data.status == 4) {
              this.$vux.toast.text(
                "您的企业已冻结<br/>客服：400-819-6985",
                "center"
              );
            } else if (res.data.errCode == "001002140") {
              window.location.href =
                this.$root.urlPath.MCMHALL + "/cert?newpage=newpage";
            }
          })
          .catch(function(error) {
            alert("账户异常" + error);
...
.flagshop .searchHeader.headerColor_F {
  color: #fff;
}

.flagshop .searchHeader.headerBgColor {
  background: #eeeeee;
  color: #333333;
}
...
.titleBgUrl1 {
  background: url(../../../static/img/flagship/title1.png) no-repeat center
    center;
  background-size: 3.7rem 0.63rem;
}

.titleBgUrl2 {
  background: url(../../../static/img/flagship/title2.png) no-repeat center
    center;
  background-size: 3.7rem 0.63rem;
}

.titleBgUrl3 {
  background: url(../../../static/img/flagship/title3.png) no-repeat center
    center;
  background-size: 3.7rem 0.63rem;
}

.titleBgUrl4 {
  background: url(../../../static/img/flagship/title4.png) no-repeat center
    center;
  background-size: 3.7rem 0.63rem;
}
